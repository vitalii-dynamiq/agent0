...
export default function OperationsDecisionDetail({ params }: PageProps) {
                ...
                <div className="space-y-2">
                  {decision.precedents.map((precedent) => (
                    <Button
                      key={precedent.id}
                      type="button"
                      onClick={() => precedent.precedentDecisionId && router.push(`/operations/decision/${precedent.precedentDecisionId}`)}
                      variant="outline"
                      size="sm"
                      className={cn(
