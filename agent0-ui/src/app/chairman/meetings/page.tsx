    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Meetings</h1>
        </div>
        <p className="text-muted-foreground">
          AI-prepared briefings for your upcoming meetings
        </p>
      </div>
...
function MeetingCard({ meeting, showDate }: MeetingCardProps) {
  ...
  return (
    <Link href={`/chairman/meeting/${meeting.id}`}>
      <div className="p-4 sm:p-5 border border-border rounded-xl transition-all duration-200 card-glow bg-card group">
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="hidden sm:flex items-center gap-4">
            <StakeholderAvatarGroup 
              ...
              size="sm"
            />
            <ChevronRightIcon className="w-5 h-5 text-muted-foreground group-hover:text-navy transition-colors" />
          </div>
        </div>
      </div>
    </Link>
